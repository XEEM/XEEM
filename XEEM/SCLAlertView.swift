...
public class SCLAlertView: UIViewController {
    ...
    // showInfo(view, title, subTitle)
    public func showInfoWithAppColor(title: String, subTitle: String, closeButtonTitle:String?=nil, duration:NSTimeInterval=0.0, colorStyle: UInt=0xDF352E, colorTextButton: UInt=0xFFFFFF) -> SCLAlertViewResponder {
        return showEmergencyDialog(subTitle)
    }
    ...
    public func showEmergencyDialog(subTitle: String)-> SCLAlertViewResponder {
        let colorStyle : UInt=0xDF352E
        let textColorStyle : UInt=0xFFFFFF
        selfReference = self
        view.alpha = 0
        let rv = UIApplication.sharedApplication().keyWindow! as UIWindow
        rv.addSubview(view)
        ...
        iconImage = SCLAlertViewStyleKit.imageOfInfo
        // Title
        self.labelTitle.text = "Tell us your problem?"
...
//        }
        for btn in buttons {
            ...
            //if btn.titleLabel == "Cancel" {
                btn.backgroundColor = UIColor.MKColor.AppMainColor
                 btn.setTitleColor(UIColorFromRGB(textColorStyle), forState:UIControlState.Normal)
            } else {
                btn.backgroundColor = viewColor
                btn.setTitleColor(UIColorFromRGB(textColorStyle), forState:UIControlState.Normal)
        ...
        //if duration > 0 {
//            durationTimer?.invalidate()
//            durationTimer = NSTimer.scheduledTimerWithTimeInterval(0.2, target: self, selector: Selector("hideView"), userInfo: nil, repeats: false)
        //}
